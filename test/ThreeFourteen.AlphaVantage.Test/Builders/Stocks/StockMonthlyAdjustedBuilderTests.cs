...
namespace ThreeFourteen.AlphaVantage.Test.Builders.Stocks
{
    public class StockMonthlyAdjustedBuilderTests : BuilderTestsBase
    {
        ...
        {
            var timeseries = await AlphaVantage.Stocks.MonthlyAdjusted("MSFT")
                .GetAsync();

            ServiceMock.LatestParameters[ParameterFields.Symbol].ShouldBe("MSFT");
            ServiceMock.LatestParameters[ParameterFields.Function].ShouldBe(Function.Stocks.TimeSeriesMonthlyAdjusted.Value);
